...
class ReportManager:
    ...
    def export_ream_report_to_pdf(self, file_path: str, user: str = None, role: str = None) -> None:
        self.require_auth(required_role='admin')
        """Export ream records to a PDF file using ReportLab."""
        try:
            if not self._check_user_auth(user, role, required_role='admin'):
            ...
            elements.append(Paragraph("Ream Records Report", styles['Title']))
            elements.append(Paragraph(f"Generated by {user or self.get_current_username() or 'unknown'} on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
            elements.append(Spacer(1, 12))

            data = [["Record ID", "Admission No", "Name", "Form", "Stream",
