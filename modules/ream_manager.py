...
class ReamManager:
    ...
    def _check_stock_alert(self, conn: sqlite3.Connection) -> tuple[int, int]:
        ...
        cursor.execute("SELECT current_balance FROM ream_stock_summary_table ORDER BY summary_id DESC LIMIT 1")
        result = cursor.fetchone()
    ...
    def get_total_reams(self, user: str, role: str) -> int:
        ...
        try:
            ...
            try:
                ...
                result = cursor.fetchone()
                total_reams = result[0] if result else 0
                self._log_audit('get_total_reams', user, 'ream_stock_summary_table', None,
