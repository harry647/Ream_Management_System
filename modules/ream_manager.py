...
class ReamManager:
    ...
    def export_ream_report_to_pdf(self, file_path: str, user: str, role: str) -> None:
        ...
        try:
            ...
            elements.append(Paragraph(f"Generated by: {user} on {datetime.now().strftime('%Y-%m-%d')}", styles['Normal']))
            elements.append(Spacer(1, 12))

            data = [["Record ID", "Adm No", "Name", "Form", "Stream", "Qty", "Term", "Date", "Recorded By"]]
